...
hex ::
  Parser Char
hex = do
  chr . g <$> thisMany 4 (satisfy isHexDigit)
  where
    g = \s -> case readHex s of
      Empty -> 0
      Full s' -> s'
...
eof ::
  Parser ()
eof =
  P (\i -> if null i then ExpectedEof i else Result i ())
  where
    null Nil = True
    null _ = False
...
satisfyAll ps = do
  c <- character
  if ((and <$>) . sequence) ps c then valueParser c else unexpectedCharParser c
...
satisfyAny ps = do
  c <- character
  if ((or <$>) . sequence) ps c then valueParser c else unexpectedCharParser c
...
betweenSepbyComma ::
  ...
  Parser a ->
  Parser (List a)
betweenSepbyComma o e pa =
  betweenCharTok o e (sepby pa (charTok ','))
